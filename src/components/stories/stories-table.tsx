...
const columns: ColumnDef<StoryDetail>[] = [
  ...
  { accessorKey: "title", header: ({ column }) => <DataGridColumnHeader column={column} title="Title" />, cell: ({ row }) => <span className="block truncate font-medium">{row.getValue("title")}</span> },
...
interface StoriesTableProps { stories: StoryDetail[]; }
...
export function StoriesTable({ stories }: StoriesTableProps) {
  ...
  return (
    <DataGrid table={table} recordCount={stories.length} tableLayout={{ dense: true, headerSticky: true, headerBackground: true, headerBorder: true, rowBorder: true }} tableClassNames={{ headerRow: "bg-muted/55", bodyRow: "hover:bg-info/5" }}>
